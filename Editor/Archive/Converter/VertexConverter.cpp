...
namespace Nippon
{
	std::vector<DefaultVertex> VertexConverter::ScrToDefault(Entity* Entity, std::vector<ScrVertex> const& Vertices, std::vector<TextureMap> const& TextureMaps, std::vector<ScrUv> const& TextureUvs, std::vector<ColorWeight> const& ColorWeights)
	{
		...
		for (U16 i = 0; i < vertexCount; i++)
		{
			...
			vertices[i] = DefaultVertex{ position, textureMap, textureUv, colorWeight, Entity->GetUniqueId() };
		}

		return vertices;
	}

	std::vector<DefaultVertex> VertexConverter::MdToDefault(Entity* Entity, std::vector<MdVertex> const& Vertices, std::vector<TextureMap> const& TextureMaps, std::vector<MdUv> const& TextureUvs, std::vector<ColorWeight> const& ColorWeights)
	{
		std::vector<DefaultVertex> vertices = {};
