...
static int FillInitData(
  ...
  _In_ size_t maxsize,
  _In_ size_t bitSize,
  _In_reads_bytes_(bitSize) const std::uint8_t* bitData,
  _Out_ size_t& twidth,
...
//--------------------------------------------------------------------------------------
static int CreateTextureFromDDS(
  const DDS_HEADER* header,
  const std::uint8_t* bitData,
  size_t bitSize,
  size_t& width,
  size_t& height,
  char unsigned** pixelBuffer
)
{
  width = header->width;
  ...
  FillInitData(width, height, depth, mipCount, arraySize, format, 0, bitSize, bitData, twidth, theight, tdepth, skipMip, initData.get());

  *pixelBuffer = (char unsigned*)calloc(width * height, 1);
  memcpy(*pixelBuffer, initData->pSysMem, width * height);
}
...
int CreateDDSTextureFromMemory(const char unsigned* ddsData, size_t ddsDataSize, size_t& width, size_t& height, char unsigned** pixelBuffer)
{
  ...
  const DDS_HEADER* header = reinterpret_cast<const DDS_HEADER*>(ddsData + sizeof(std::uint32_t));
  ...
  ptrdiff_t offset = sizeof(std::uint32_t) + sizeof(DDS_HEADER) + (bDXT10Header ? sizeof(DDS_HEADER_DXT10) : 0);

  return CreateTextureFromDDS(header, ddsData + offset, ddsDataSize - offset, width, height, pixelBuffer);
}