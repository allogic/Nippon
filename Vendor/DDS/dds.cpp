...
void dds_parse_uncompressed(dds_image* image, const char* data, long data_length)
{
...
void dds_parse_dxt1(dds_image* image, const char* data, long data_length)
{
	dds_uint img_width = MAX(1, image->header.width);
	dds_uint img_height = MAX(1, image->header.height);
	dds_uint img_depth = MAX(1, image->header.depth);
...
void dds_parse_dxt3(dds_image* image, const char* data, long data_length)
{
	dds_uint img_width = MAX(1, image->header.width);
	dds_uint img_height = MAX(1, image->header.height);
	dds_uint img_depth = MAX(1, image->header.depth);
...
void dds_parse_dxt5(dds_image* image, const char* data, long data_length)
{
...
void dds_parse_dxt10(dds_image* image, const char* data, long data_length)
{
...
dds_image* dds_load_from_memory(const char* data, long data_length)
{
	...
	dds_image* ret = (dds_image*)malloc(sizeof(dds_image));


	// read the header
	memcpy(&ret->header, data_loc, sizeof(dds_header));
	data_loc += sizeof(dds_header);
	...
	if ((ret->header.pixel_format.flags & DDPF_FOURCC) && ret->header.pixel_format.four_cc == FOURCC("DX10")) {
		// read the header10
		memcpy(&ret->header10, data_loc, sizeof(dds_header_dxt10));
		data_loc += sizeof(dds_header_dxt10);
	}
	...
	if ((ret->header.pixel_format.flags & DDPF_FOURCC) && ret->header.pixel_format.four_cc == FOURCC("DXT1"))
		dds_parse_dxt1(ret, data_loc, data_length - ((long)(data_loc - data)));
	else if ((ret->header.pixel_format.flags & DDPF_FOURCC) && ret->header.pixel_format.four_cc == FOURCC("DXT2"))
		dds_parse_dxt3(ret, data_loc, data_length - ((long)(data_loc - data)));
		...
		dds_parse_dxt5(ret, data_loc, data_length - ((long)(data_loc - data)));
		...
		dds_parse_dxt10(ret, data_loc, data_length - ((long)(data_loc - data)));
	}
		...
		dds_parse_uncompressed(ret, data, data_length - ((long)(data_loc - data)));

	return ret;
}

void dds_image_free(dds_image* image)
{
	free(image->pixels);
	free(image);
}